...
| [x] | T2.1 | Dataset preparation | T1.2 | `configs/llvip.yaml`, `reports/dataset_prep.md`; 15,486 imgs at 160x120 sensor-emulated (blur σ=0.8, noise 2DN, min_h=4px) |
| [x] | T2.2 | Baseline training (local) | T2.1 | Fine-tuned YOLOv8n 30/30 epochs at 160px sensor-res on GTX 1650; val mAP50=0.8532 |
| [x] | T2.3 | Evaluation harness | T2.2 | `reports/eval_yolov8n_llvip.json` **at 160x120 sensor res**: mAP50=61.4%, recall=59.3%, P=69.4%; `reports/eval_recall_by_bucket.md` (near 95.7%, far 56.9%) |
| [x] | T2.4 | ONNX export + wrapper | T2.2 | `models/yolov8n_llvip.onnx`, `onnx_detector.py`; dynamic input shape (160x160), CPU latency=2.75ms; 3 tests pass |
| [x] | T2.5 | Threshold tuning | T2.3, T2.4 | `reports/threshold_sweep.md` at 160x120; recommended conf=0.10, iou=0.30 (F1=0.6403) |
...
| [x] | T3.3 | Reprojection evaluation | T3.2, T1.2 | `reports/reprojection_eval.md` (corrected — predictor-based, not oracle); slow_scan@150ms benefit=+17.0, fast_turn@150ms benefit=+75.1 |
...
| [x] | T4.3 | Tracker on real detections | T4.2, T2.4 | `reports/tracking_eval.json`; ID switches and track fragmentation tracked |
| [x] | T5.1 | Hot-scene handling | T1.2 | `reports/agc_eval.md` generated; adaptive CLAHE RMS=58.2 vs naive=51.3 on real LLVIP frames |
...
| [x] | T6.3 | Safety monitor & fault injection | T6.2 | `src/yaazhi/safety/monitor.py`, `faults.py`; 4 tests pass; state machine NORMAL->DEGRADED->SAFE & fault injection working |
| [~] | T6.4 | Alert messaging & command post | T6.2 | UDP lossy binary alert protocol (can complete next) |
| [x] | T7.1 | Expo application | T6.2, T6.3, T5.1 | `src/yaazhi/app/expo_app.py`, `scripts/run_demo.py`; interactive split-screen demo with hotkeys R, S, H, F, 1-5, SPACE, C, Q |
